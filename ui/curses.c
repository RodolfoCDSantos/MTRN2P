    ...
    struct mtr_ctl *ctl)
{
    ...
    move(0, 0);
    attron(A_BOLD);
    /* snprintf(buf, sizeof(buf), "NET2PHONE TRACE ROUTE"); */
    mvprintw(0, 4, "NET2PHONE TRACE ROUTE");
    mvprintw(0, maxx - 43, "Current time: %s GMT", iso_time(&t)); 
    snprintf(buf, sizeof(buf), "Simulating a UDP PCM audio channel with 1500 bit payload, from: %s (%s), to: %s. \n", ctl->LocalHostname, net_localaddr(), ctl->Hostname);
    pwcenter(buf);
    attroff(A_BOLD);
    mvprintw(2, 4, " ");

        t = time(NULL);
    /* mvprintw(1, maxx - 43, "Current time: %s GMT", iso_time(&t)); */
    mvprintw(3, 4, " ");
    printw("\n");
    move(44, 0);
    mvprintw(48, 2, "Type d to change display, r to reset statistics or q to quit.");
    mvprintw(48, maxx - 29, "NET2PHONE Voice Engineering.");
    printw("\n");
    ...
    if (ctl->display_mode == DisplayModeDefault) {
        ...
        attron(A_BOLD);
        mvprintw(rowstat - 1, 3, " HOST TRACKING");
